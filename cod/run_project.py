...
params.overlap = 0.3
params.number_positive_examples = 2 * 6713  # numarul exemplelor pozitive
...
params.threshold_mining = 0

params.has_annotations = True

params.scaling_ratio = 0.9  # nu am folosit in forma aceasta
...
if params.use_hard_mining:
    hard_features_path = os.path.join(params.dir_save_files, 'descriptoriExempleHard_' + str(params.dim_hog_cell) + '_'
                                      + str(params.number_negative_examples) + '.npy')
    if os.path.exists(hard_features_path):
        hard_features = np.load(hard_features_path)
        print('Am incarcat descriptorii pentru exemplele hard negative')
    else:
        print('Construim descriptorii pentru exemplele hard negative:')
        # rulez sliding window pe exemplele negative si adaug ferestrele fals pozitive la setul de date, apoi antrenez
        # un nou SVM cu toate datele
        hard_features = facial_detector.negative_mining()
        np.save(hard_features_path, hard_features)
        print('Am salvat descriptorii pentru exemplele hard negative in fisierul %s' % hard_features_path)
    training_examples = np.concatenate((training_examples, np.squeeze(hard_features)), axis=0)
    train_labels = np.concatenate((train_labels, np.zeros(hard_features.shape[0])))
    facial_detector.train_classifier(training_examples, train_labels)
